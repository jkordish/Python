...
parser.add_option("-d", "--debug", help="Debug mode: check /var/log/vCenter_Metrics_Query.log", dest="debug",
    action="store_true")
...
cnxn = pyodbc.connect(
    "Driver=TDS;Servername=vCenter;UID=%s;PWD=%s;" % (options.username, base64.decodestring(options.password)))
cursor = cnxn.cursor()
...
order by NAME, SAMPLE_TIME DESC
    ...
    for row in rows:
        if options.debug is True:
            ...
            output.write(
                '/opt/OV/bin/opcmon FLED_VirtualMachine_CPU=%s -object "Perf" -option NODE="%s"\n' % (row.CPU, row.VM))
        if vmname != row.VM:
            os.system(
                '"/opt/OV/bin/opcmon" FLED_VirtualMachine_CPU=%s -object "Perf" -option NODE="%s"' % (row.CPU, row.VM))
            vmname = row.VM
...
order by NAME, SAMPLE_TIME DESC
    ...
    for row in rows:
        if options.debug is True:
            ...
            output.write(
                '/opt/OV/bin/opcmon FLED_VirtualMachine_MEM=%s -object "Perf" -option NODE="%s"\n' % (row.MEM, row.VM))
        if vmname != row.VM:
            os.system(
                '"/opt/OV/bin/opcmon" FLED_VirtualMachine_MEM=%s -object "Perf" -option NODE="%s"' % (row.MEM, row.VM))
            vmname = row.VM
...
Order by VM
    ...
    for row in rows:
        ...
        if options.debug is True:
            ...
            output.write(
                '/opt/OV/bin/opcmon FLED_VirtualMachine_DISKFREE=%s -object "Perf" -option NODE="%s" -option PATH="%s"\n' % (
    ...
    for row in rows:
        if options.debug is True:
            ...
            output.write('/opt/OV/bin/opcmon FLED_VirtualMachine_CPUWAIT=%s -object "Perf" -option NODE="%s"\n' % (
                row.WAIT, row.VM))
        if vmname != row.VM:
            os.system('"/opt/OV/bin/opcmon" FLED_VirtualMachine_CPUWAIT=%s -object "Perf" -option NODE="%s"' % (
                row.WAIT, row.VM))
            vmname = row.VM
    ...
    for row in rows:
        if options.debug is True:
            ...
            output.write('/opt/OV/bin/opcmon FLED_VirtualMachine_DISKIO=%s -object "Perf" -option NODE="%s"\n' % (
                row.IO, row.VM))
        if vmname != row.VM:
            os.system('"/opt/OV/bin/opcmon" FLED_VirtualMachine_DISKIO=%s -object "Perf" -option NODE="%s"' % (
                row.IO, row.VM))
            vmname = row.VM
